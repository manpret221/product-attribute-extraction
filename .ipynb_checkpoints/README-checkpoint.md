...
Regenerate: `python3 -m src.generate_dataset`
...
### Demo script
`python3 demo.py` runs the pipeline on brief sample descriptions locally
...
Run `python3 -m src.evaluate`. It evaluates the **full hybrid pipeline**
...
dataset shares vocabulary with the extractor's dictionary by design (see
Section 1) — they mainly validate that the pipeline is implemented
correctly, not that it generalizes to arbitrary catalog copy. Section 5
